...
def add_book():
    ...
    print(data)
    user_identity = get_jwt_identity()
    user = db.session.scalar(sa.select(User).where(User.id == user_identity))
    flask_app.logger.debug(f"User identity from JWT: {user_identity}")

    title = data.get('title')
    author = data.get('author')
    genre = data.get('genre', "Not available")
    ...
    book = db.session.scalar(sa.select(Book).where(Book.title == title))
    ...
    if not book:
        ...
        try:
            db.session.commit()
            print("New book committed to the database.")
        except Exception as e:
            db.session.rollback()
            print("Error committing book:", e)
